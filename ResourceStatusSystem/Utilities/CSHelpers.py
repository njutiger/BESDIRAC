...
def getSiteVO( siteName ):
    ...
    for domainName in domainNames:
        result = gConfig.getSections( '%s/%s/%s' % ( _basePath, domainName, siteName ) )
        if result[ 'OK' ]:
            siteDomain = domainName
            break
    
    if siteDomain is None:
        return S_ERROR( "Can't find site %s at %s section" % ( siteName, _basePath ) )
    
    vos = gConfig.getValue( '%s/%s/%s/VO' % ( _basePath, siteDomain, siteName ) )
    if vos is None:
        ces = gConfig.getSections( '%s/%s/%s/CEs' % ( _basePath, siteDomain, siteName ) )
        if ces[ 'OK' ]:
            ces = ces[ 'Value' ]
            vos = gConfig.getValue( '%s/%s/%s/CEs/%s/VO' % ( _basePath, siteDomain, siteName, ces[ 0 ] ) )
            if vos is None:
                queues = gConfig.getSections( '%s/%s/%s/CEs/%s/Queues' % ( _basePath, siteDomain, siteName, ces[ 0 ] ) )
                if queues[ 'OK' ]:
                    queues = queues[ 'Value' ]
                    vos = gConfig.getValue( '%s/%s/%s/CEs/%s/Queues/%s/VO' % ( _basePath, siteDomain, siteName, ces[ 0 ], queues[ 0 ] ) )
    
    if vos:
        return S_OK( vos.split( ',' ) )
    return S_OK( [] )
