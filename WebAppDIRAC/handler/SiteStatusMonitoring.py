...
class SiteStatusMonitoring( WebHandler ):
    ...
    def web_getSAMData( self ):
        samInfo = [{ 'ElementName' : 'chenj01.ihep.ac.cn', 'ElementType' : 'CE', 'WMS' : 'OK', 'CVMFS' : 'OK', 'BOSS' : '-' }]
        samInfo = json.dumps(samInfo);
        self.finish( { 'success' : 'ture', 'result' : samInfo, 'total' : len( samInfo ) } )
    ...
    @asyncGen        
    def web_getExpandData( self ):
        publisher = RPCClient( 'ResourceStatus/Publisher' )
        
        req = self._request()
        site = req[ 'Site' ][ -1 ].encode()
        
        columns = [ 'ElementName', 'ElementType', 'Tests', 'SAMStatus', 'LastUpdateTime' ]
        samDetail = yield self.threadTask( publisher.getSiteSAMDetailWeb, site, columns )
        if not samDetail[ 'OK' ]:
            self.finish( { 'success' : 'false', 'result' : [], 'total' : 0, 'error' : samDetail[ 'Message' ] } )
            return
        samDetail = samDetail[ 'Value' ]
            
        self.finish( { 'success' : 'true', 'result' : samDetail, 'total' : len( samDetail ) } )
