...
def getSiteVO( siteName ):
  ...
  if domain == 'CLOUD':
    _basePath = 'Resources/VirtualMachines'
    pods = gConfig.getSections( '%s/RunningPods' % _basePath )
    if pods[ 'OK' ]:
      pods = pods[ 'Value' ]
      for pod in pods:
        endpoint = gConfig.getValue( '%s/RunningPods/%s/CloudEndpoints' % ( _basePath, pod ) )
        site = gConfig.getValue( '%s/CloudEndpoints/%s/siteName' % ( _basePath, endpoint ) )
        if site is not None and site == siteName:
          groups = gConfig.getValue( '%s/RunningPods/%s/Requirements/OwnerGroup' % ( _basePath, pod ) )
